...
To give one second mate a different launch profile from the rest, write the same line to `config/secondmate-harness.d/<id>`; [`docs/configuration.md`](../../../docs/configuration.md) owns its precedence, and every spawn, recovery, liveness respawn, and relaunch of that id honors it.
