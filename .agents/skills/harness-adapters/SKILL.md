...
`../../../bin/fm-harness.sh secondmate [<id>]` resolves the id's `config/secondmate-harness.d/<id>` profile -> `config/secondmate-harness` -> `config/crew-harness` -> firstmate's own harness.
