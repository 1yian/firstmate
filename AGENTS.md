...
config/secondmate-harness  harness the PRIMARY uses to launch SECONDMATE agents, optionally followed by a model and effort token on the same line ("<harness> [<model>] [<effort>]"; section 4); LOCAL, gitignored; absent or "default" harness falls back to config/crew-harness then firstmate's own. The primary's own setting; NOT inherited into secondmate homes (secondmates do not spawn secondmates). Optional per-secondmate override config/secondmate-harness.d/<id>, same format and same non-inheritance (docs/configuration.md)
...
Query `bin/fm-project-mode.sh --branch-prefix <project>` at intake; `-` means no fixed branch, so omit `--branch-prefix` and let the worker choose and record its conventional branch. Pass a registered prefix, including an explicitly empty one, to the brief, ship spawn, or scout promotion as `--branch-prefix`.
