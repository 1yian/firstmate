...
# default-branch base, the recorded task branch (bin/fm-task-branch-lib.sh), and - rendered from
...
# contract is decided: --mode and --yolo are written into the meta alongside
# the kind= flip. An explicit --branch-prefix also fixes and records the branch;
# otherwise the worker chooses and records a conventional-commit branch after
# promotion. Firstmate resolves the posture at promotion time, having just
...
if [ "$BRANCH_PREFIX_SET" -eq 1 ]; then
  PROMOTION_BRANCH_ACTION="create your branch: \`git checkout -b $BRANCH_Q --\` (the selected project prefix is already recorded for this task)."
fi
