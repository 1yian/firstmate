...
# Without --branch-prefix the worker chooses and records a conventional-commit
# <type>/<slug> branch through bin/fm-task-branch.sh. An explicit
# --branch-prefix <prefix> instead fixes the branch as <prefix><task-id> before
# dispatch. Pass an empty prefix for a bare <task-id> branch. Older tasks with
# no recorded branch retain their plain <task-id> and fm/<task-id> fallbacks. Like --mode, this script never reads data/projects.md for it: the
...
BRANCH_ACTION=$(fm_task_branch_first_action "$FM_ROOT" "$STATE/$ID.meta")
if [ "$BRANCH_PREFIX_SET" -eq 1 ]; then
  BRANCH_ACTION="create your branch: \`git checkout -b $BRANCH_Q --\`"
fi
...
1. First action: $BRANCH_ACTION$SETUP2
