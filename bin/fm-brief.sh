...
Your own machinery delivers the durable facts about your crew's work for you (\`docs/secondmate-parent-channel.md\`): a child's PR-ready or terminal line, a scout report, a failed line, and a decision or blocker left open past its threshold are mirrored to the parent channel by \`bin/fm-parent-mirror.sh\` on every supervision poll, \`bin/fm-pr-check.sh\` and \`bin/fm-captain-hold.sh\` publish the PR and captain-hold facts they record, and a merge is reported by the merge outcome path.
What only you can append is judgement: the answer to a marked request below, a recommendation or caveat on a mirrored outcome, a blocker or failure of your own, and anything else you would otherwise say to the captain.
