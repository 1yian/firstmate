...
fm_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle>
  ...
  local before after verdict caps cy identity
  ...
  before=$(fm_tmux_composer_capture "$target") \
    || { printf 'unknown'; return 0; }
  ...
  sleep "$settle"
  after=$(fm_tmux_composer_capture "$target") \
    || { printf 'unknown'; return 0; }
  cy=$(fm_tmux_composer_cursor_row "$target") || { printf 'unknown'; return 0; }
  if ! identity=$(fm_tmux_composer_identity "$target"); then
    identity=probe-absent
  fi
  if ! verdict=$(fm_composer_pre_enter_verdict "$after" "$text" "$before" "$caps" "$cy" "$identity"); then
    printf '%s' "$verdict"
    return 0
  fi
