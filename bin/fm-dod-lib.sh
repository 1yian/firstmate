...
fm_ship_rule_one() {  # <no-mistakes|direct-PR|local-only> <task-id> [branch] [<forge>]
  ...
  [ -n "$branch" ] || branch='your task branch'
  ...
  case "$mode" in
    direct-PR)
      if [ "$branch" = 'your task branch' ]; then
        printf '%s\n' '1. Never push to the default branch (push only your task branch). Never merge a PR.'
      else
    ...
    local-only)
      if [ "$branch" = 'your task branch' ]; then
        printf '%s\n' "1. Never push to any remote and never open a PR. Work only on your task branch; firstmate handles the merge into local \`main\`."
      else
...
fm_dod_block() {  # <mode> <task-id> [branch] [<forge>]
  local mode=$1 id=$2 forge=${4:-none}
  local branch=${3:-} branch_line='' ready_branch
  fm_forge_valid_for_mode "$forge" "$mode" fm_dod_block || return 1
  if [ -n "$branch" ]; then
    branch_line="Ship branch: $branch"
    ready_branch=$branch
  else
    branch='your task branch'
    ready_branch='{your task branch}'
  fi
  case "$mode:$forge" in
    direct-PR:gerrit)
      cat <<EOF
...
When it is implemented and committed, append \`done [at=<epoch>]: ready in branch $ready_branch\` to the status file and stop.
