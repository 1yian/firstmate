...
#   Without --branch-prefix the worker chooses a conventional-commit branch and
#   records it through bin/fm-task-branch.sh. An explicit --branch-prefix fixes
...
if [ "$RELAUNCH" -eq 0 ] && [ "$KIND" = ship ] && [ "$BRANCH_PREFIX_SET" -eq 1 ]; then
...
if [ "$KIND" = ship ]; then
  ...
  if [ -n "$BRIEF_BRANCH" ]; then
    ...
    : # New task: its worker chooses and records a conventional branch.
  elif [ "$RELAUNCH" -eq 1 ]; then
    # The worker may have chosen its conventional branch after the brief was written.
    echo "notice: $BRIEF records no ship branch; relaunching on the recorded branch $BRANCH" >&2
  else
  ...
  if [ -n "${BRANCH:-}" ] && [ "$STANDING_BRANCH" != '-' ] && [ "$BRANCH" != "$STANDING_BRANCH$ID" ]; then
