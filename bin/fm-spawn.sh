...
# Claude launch-readiness: a freshly created worktree parks on Claude Code's
# workspace-trust dialog, which every idle/alive read treats as healthy. Poll
# until the pane is an accepting composer; a gated modal fails the spawn with
# the dialog text attached rather than returning a route to a parked agent.
claude_wait_for_ready() {
  ...
  while [ "$i" -lt "$max" ]; do
    ...
    # Firstmate's own test stubs often omit a rendered composer. A gated
    # dialog still fails above; an empty capture is treated as a stub with no
    # TUI rather than a launch hang. Production never sets this bypass.
