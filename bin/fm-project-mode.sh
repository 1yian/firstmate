...
# --branch-prefix prints the project's registered fixed ship-branch prefix.
# A single '-' means no branch= annotation: let the worker choose and record
# a conventional-commit branch. An explicitly empty branch= prints an empty line,
# selecting a fixed bare task-id branch instead.
...
# yolo, and any explicit ship-branch prefix are resolved by firstmate at intake;
# only a registered fixed prefix is passed to bin/fm-brief.sh, bin/fm-spawn.sh,
# and bin/fm-promote.sh (AGENTS.md
...
#   - <name> - <desc> (added <date>)                                 -> no-mistakes off -  (worker choice)
...
#   ("branch=") resolves to "" for a bare "<task-id>" ship branch instead of
#   the default worker-chosen branch.
#
# Registered modes:
...
# branch=<prefix> (orthogonal) = selects a fixed <prefix><task-id> branch instead
#   of the worker's conventional name, e.g. for a third-party repo. Query with
...
# "-" under --branch-prefix) and warns to stderr, so a typo never silently
...
# the raw prefix, defaulting to "-"; forge is `none` or the whole `forge=<value>`
...
parsed=$(awk -v n="$NAME" '
  ...
  }
  {
    ...
    mode="no-mistakes"; yolo="off"; branch="-"; forge="none";
    if (substr(after, 1, 2) == " [") {
      s="";
      nk = split(after, rest, " ");
...
case "$mode" in
  ...
  *) echo "warn: unknown mode \"$mode\" for $NAME; defaulting to no-mistakes off" >&2; mode=no-mistakes; yolo=off; branch=- ;;
esac
