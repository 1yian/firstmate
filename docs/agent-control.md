...
1. **Resolve the profile.**
   ...
   Otherwise a `kind=secondmate` task re-resolves its durable secondmate pin (its own `config/secondmate-harness.d/<id>` profile when present, otherwise `config/secondmate-harness`; [`docs/configuration.md`](configuration.md) owns the precedence), including that file's optional model and effort tokens, exactly as every other respawn does - so setting the pin and relaunching is the ordinary way to move a secondmate's runtime.
