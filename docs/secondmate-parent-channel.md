...
4. `bin/fm-captain-hold.sh` publishes `needs-decision [key=captain-hold-<task>-<occurrence>]` when it holds a task for the captain in a secondmate home and the matching occurrence-keyed `resolved` line when the answer is recorded, including batch answers and idempotent retries.
   Each hold writes a durable generation into the task body and each resolution records the generation it answers, so a re-held task opens and closes a distinct parent decision even when the captain repeats the same words, while an exact same-generation retry remains one delivery.
