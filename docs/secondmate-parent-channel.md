...
4. `bin/fm-captain-hold.sh` publishes `needs-decision [key=captain-hold-<task>-<occurrence>]` when it holds a task for the captain in a secondmate home and the matching occurrence-keyed `resolved` line when the answer is recorded, including batch answers and idempotent retries.
   Each hold writes a durable generation into a script-owned block anchored at the start of the task body, each resolution records the generation it answers in a length-framed block with explicit begin and end delimiters, and fallback counting accepts only complete delimited blocks, so unrestricted captain prose cannot impersonate lifecycle state, a re-held task opens and closes a distinct parent decision even when the captain repeats the same words, and an exact same-generation retry remains one delivery.
