...
4. `bin/fm-captain-hold.sh` publishes `needs-decision [key=captain-hold-<task>-<occurrence>]` when it holds a task for the captain in a secondmate home and the matching occurrence-keyed `resolved` line when the answer is recorded, including batch answers and idempotent retries.
   Each hold writes a durable generation into a script-owned block anchored at the start of the task body, each resolution records the generation it answers in its own structured header, and legacy fallback counting accepts only structurally valid resolution headers, so unrestricted captain prose cannot impersonate lifecycle state, a re-held task opens and closes a distinct parent decision even when the captain repeats the same words, and an exact same-generation retry remains one delivery.
5. `bin/fm-teardown.sh` performs the child's final sweep before it removes the child's record, then retires the child's mirror state; an undelivered final sweep must leave a verified durable orphan record before the discoverable child record is removed, otherwise teardown refuses so a later poll or retry can still find the child.
