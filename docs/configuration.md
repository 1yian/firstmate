...
A wake is delegated only when every unread queue row is task-local and every task's project is listed.
Fleet-wide, unresolvable, mixed-scope, absent, unreadable, empty, legacy `on`, `off`, or otherwise malformed grants stay on the captain-facing main path and activate no branch-owned runtime state or lease cleanup.
...
This grant is local to each Firstmate home and is not part of secondmate inherited configuration; each home remains disabled until its captain explicitly writes at least one valid `project=<value>` grant.
