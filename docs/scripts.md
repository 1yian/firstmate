...
| [`fm-task-branch-lib.sh`](../bin/fm-task-branch-lib.sh) | Own worker-chosen conventional-commit branch names, recorded branch resolution, and plain-id or legacy `fm/<id>` fallback |
| [`fm-task-branch.sh`](../bin/fm-task-branch.sh) | Worker-run: create and record a worker-chosen task branch, refusing a taken name |
