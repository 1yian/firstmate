...
It also covers generation-claim single-flight and supersession: a live open ledger claim defers concurrent firings with no lock held, a finished, dead, identity-mismatched, or stuck claim is superseded so a lapsed home re-arms, and a superseded owner goes silent instead of double-translating; the legacy lock-holding claim shapes from pre-generation builds keep their defer-or-reclaim coverage through the legacy shim ([`turnend-guard.md`](turnend-guard.md) owns that boundary).
`FM_CLAUDE_LIVE_E2E=1 tests/fm-claude-stop-autoarm-live-e2e.test.sh` starts with the reproduced stale-lock state, runs session start first, completes two tokenless cycles, and checks the competing-live-owner negative control.
`tests/fm-turnend-guard.test.sh` covers the cooperative `--claude` guard, including monotonic failed-epoch progression, the integrated bounded fail-open, post-alarm continuation suppression, and positive recovery reset; [`turnend-guard.md`](turnend-guard.md#regression-coverage) lists that suite's full generation-claim and legacy-shim coverage.
