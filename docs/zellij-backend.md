...
Before sending Enter, the adapter rejects a gated pre-type screen, then proves that the selected composer's normalized content changed and contains the payload's own tail; an unreadable composer, a paste that lands elsewhere, or unrelated pane output fails without submitting.
