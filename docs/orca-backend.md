...
`fm-send.sh` first requires the plain capture to prove an empty composer, then types once and requires the selected composer to change and contain the payload's own tail before sending Enter.
It verifies composer clearance through the fleet-wide classifier in `bin/fm-composer-lib.sh`, retrying Enter without retyping when a slash popup first fills an argument placeholder.
