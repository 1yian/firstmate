...
Ship briefs also tell the crewmate to verify `pwd -P` and `git rev-parse --show-toplevel` before creating the task branch - the plain task-id slug, with older tasks keeping a legacy `fm/<id>` branch ([`fm-task-branch-lib.sh`](../bin/fm-task-branch-lib.sh) owns naming and resolution) - then stop with a blocked status if it landed in the primary checkout.
