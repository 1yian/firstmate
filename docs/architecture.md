...
One wake covers every status byte appended since the previous one, so the main-session watcher judges the batch rather than only the file's newest line: `last_status_line` answers what a crew currently declares, while the captain-relevant record identifies the latest still-live event that firstmate must be shown and distinguishes repeated identical events by occurrence count.
The per-wake triage and heartbeat recovery paths use that record, so an unsurfaced decision, review-ready PR, blocker, terminal release or install completion, or unreadable status file surfaces instead of being absorbed, while an already surfaced event stays quiet after later routine appends and a readable routine-only status remains routine.
The record preserves the status line's free-form bytes, including consecutive and trailing tabs, while the shared fleet scanner retains newest-line reading by default so the away-mode daemon's catch-all behavior remains unchanged.
