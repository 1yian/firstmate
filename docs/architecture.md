...
The per-wake watcher triage and the main-session fleet catch-all opt into the second question, so an unsurfaced decision, review-ready PR, blocker, or terminal release or install completion followed by any routine line can no longer classify as routine and stall with no supervisor turn.
