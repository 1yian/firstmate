...
The deeper session-start agent-process liveness probe is separate from that busy-state poll: tmux and Herdr have verified classifiers for secondmate recovery, while Zellij, Orca, and cmux do not, so a dead secondmate on those backends is reported inconclusive rather than auto-respawned.
