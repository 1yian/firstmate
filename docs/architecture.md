...
Ship briefs also tell the crewmate to verify `pwd -P` and `git rev-parse --show-toplevel` before creating the task branch - a conventional-commit style `<type>/<slug>` the worker chooses and records in the task metadata through [`fm-task-branch.sh`](../bin/fm-task-branch.sh), with older tasks keeping a plain `<id>` or legacy `fm/<id>` branch ([`fm-task-branch-lib.sh`](../bin/fm-task-branch-lib.sh) owns naming and resolution) - then stop with a blocked status if it landed in the primary checkout.
