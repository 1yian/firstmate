...
The per-wake watcher triage and the fleet catch-all both read the second question, so a decision, a review-ready PR, a blocker, or a terminal release or install completion followed by any routine line can no longer classify as routine and stall with no supervisor turn.
