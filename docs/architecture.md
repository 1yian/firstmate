...
The per-wake triage and heartbeat recovery paths use that record, so an unsurfaced decision, review-ready PR, blocker, or terminal release or install completion still surfaces when followed by routine lines, an already surfaced event stays quiet after later routine appends, and a routine-only status remains routine.
