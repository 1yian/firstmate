...
One wake covers every status byte appended since the previous one, so the watcher judges that whole batch rather than the file's newest line: `last_status_line` answers what a crew currently declares, `last_captain_relevant_status_line` answers what firstmate must be shown, and the two differ exactly when a routine append lands behind an actionable event.
The per-wake watcher triage and the main-session fleet catch-all opt into the second question, so an unsurfaced event occurrence for a decision, review-ready PR, blocker, or terminal release or install completion followed by any routine line can no longer classify as routine and stall with no supervisor turn, while occurrence identity preserves free-form status text including consecutive and trailing tabs.
The shared fleet scanner retains newest-line reading by default, so the away-mode daemon's catch-all behavior remains unchanged.
