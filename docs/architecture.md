...
The separate pre-type gate scorer in that library returns `gated` without changing the post-Enter classifier's semantics, and `fm-spawn.sh` uses the shared library for Kimi and Claude launch readiness instead of carrying another shape copy.
...
A backend without an erase primitive refuses a short payload before typing, and every outcome after a successful literal write that cannot be proven is `typed-unproven` and must never be retried automatically.
There is deliberately no bypass: a harness whose composer cannot be proven is refused rather than steered blind, and pinning a new harness or backend means running the live guards in [runtime-backends.md](verification/runtime-backends.md#pre-enter-delivery-proof).
After Enter, the existing shape-aware submit confirmation remains separately required before delivery is reported as confirmed.
