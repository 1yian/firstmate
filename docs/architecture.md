...
The per-wake signal path bounds that span by the drain's presentation cursor, so an event already presented cannot re-fire it while a repeated identical line still counts as new; the fleet backstop and the away-mode digest keep their own content dedupe.
