...
One wake covers every status byte appended since the previous one, so the main-session watcher judges the batch rather than only the file's newest line: `last_status_line` answers what a crew currently declares, while the captain-relevant record identifies the latest event that firstmate must be shown and distinguishes repeated identical events by occurrence count.
