...
Every backend submit path asks `fm_composer_pre_enter_verdict` in `bin/fm-composer-lib.sh` for the payload-bound pre-Enter proof before sending Enter, so a modal that swallows typed input cannot confirm delivery.
