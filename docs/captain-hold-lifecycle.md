...
Repeats are idempotent, a closed task is refused rather than reopened, and a task with a pending backlog close is refused before mutation so close recovery cannot retire a newly created captain call.
