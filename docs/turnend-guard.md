...
No mutex is held across arming or output: `state/.claude-autoarm.lock` survives only as a micro-mutex serializing individual ledger writes, and the owner re-verifies it still holds the newest generation before every ledger write and every emit, going completely silent when superseded, so one supersession episode produces exactly one translation.
...
A generation claim without its recorded identity is not open and cannot defer either Stop participant. A legacy build's lock-holding claim (recognizable by its `autoarm` role file) still defers or reclaims under the legacy abandonment proof so an upgrade mid-session cannot double-arm.
