...
A claim carrying no recorded identity keeps ledger-plus-liveness reasoning, and a legacy build's lock-holding claim (recognizable by its `autoarm` role file) still defers or reclaims under the legacy abandonment proof so an upgrade mid-session cannot double-arm.
