...
A processing custom message is dedicated when no real captain message was delivered with it since the preceding assistant response; otherwise it is mixed with the captain's current turn.
During a dedicated attempt, ordinary assistant text remains provisional until `fm_branch_processed` successfully acknowledges the exact highest sequence that request listed.
A state-aware Markdown transformer suppresses provisional assistant text while a dedicated attempt is streaming, without hiding finalized transcript history or mixed-turn answers.
Pi's `message_end` replacement independently removes that text before finalized transcript listeners and session persistence while preserving thinking, tool calls, usage, errors, and other non-text content; text produced after the accepted acknowledgement is retained.
A refused or partial acknowledgement does not commit that dedicated attempt's text, while a mixed turn never suppresses the captain's legitimate answer even when the old outcome remains open.
Nothing else advances the processed marker: an unrelated reply, an empty reply, or a reply that paraphrases the outcome leaves the sequence unprocessed, and the extension presents the current unprocessed sequence set again at the next main run boundary and at every session start.
...
The branch prompt's "Verdict: routine or captain" section owns the verdict criteria, including how requested work's finished results and its mere progress updates are classified; unsolicited routine outcomes stay hidden, unchanged fleet reviews remain silent, and doubt escalates.
...
A review that found literally nothing worth reporting uses verdict `routine`, `task=fleet`, and `silent=true` to retain that no-change fact in the durable record.
A fleet-wide routine action omits `silent`, and both routine shapes remain hidden from the captain transcript.
...
Portable regressions: `tests/fm-pi-branch-extension.test.sh` covers dispatch, signal and stale report scoping with unscoped heartbeat reports, repeated already-complete stale outcomes staying hidden in the captain's Pi and Herdr setup under both Calm settings, the new branch conversation at every main session start with continuation inside one session, the mirror re-anchor that pairs with it, requested-versus-unsolicited delivery, exact visible entry content, no unkeyed model turn, sequence-keyed processing and acknowledgement, provisional dedicated text removal, successful and rejected acknowledgements, mixed captain turns, retry exhaustion without renewal on sequence widening, fresh budgets after oldest-sequence progress and session replacement, routine outcomes staying turn-free, processed-marker migration, idle and busy main state, incident-shaped compaction and unrelated-assistant context, cold-start post-lock recovery, crash-before-cursor reload recovery, repeated-reload idempotency, mirroring, post-construction provider-error and no-report fallback, the consecutive-error latch, cooldown probe, exponential backoff, report-plus-settlement recovery, report-before-error re-latch, cache key, and model and effort selection.
`tests/fm-branch-supervision.test.sh` covers prompt stability, store append-only behavior, silent startup consumption of routine rows, the captain cursor barrier, the processed marker's sequence bounds, leases, guards, and non-branch-home invariance.
...
Live guard: `FM_PI_BRANCH_LIVE_E2E=1 tests/fm-pi-branch-live-e2e.test.sh` exercises the real installed Pi SDK's immediate active-transcript rendering of captain entries, suppression and persistence of hidden routine messages, custom-entry model exclusion, branch-session surfaces, watcher-owned fallback after rejected branch settlement, Markdown transformation during streaming, `message_end` replacement, stable inline and fullscreen rendering, and reopened session history.
