...
Live guard: `FM_PI_BRANCH_LIVE_E2E=1 tests/fm-pi-branch-live-e2e.test.sh` exercises the real installed Pi SDK's immediate active-transcript appendEntry rendering, persistence, custom-entry model exclusion, branch-session surfaces, watcher-owned fallback after rejected branch settlement, Markdown transformation during streaming, `message_end` replacement, stable inline and fullscreen rendering, and reopened session history.
