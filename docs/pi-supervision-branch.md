...
Pi's `message_end` replacement removes provisional text before finalized transcript listeners and session persistence while preserving thinking, tool calls, usage, errors, and other non-text content; text produced after the accepted acknowledgement is retained.
...
Pi can render generated tokens while the assistant message is still streaming, before `message_end` can replace it, so the stable inline and fullscreen transcript and reopened session are clean but a transient flash of provisional text remains an accepted SDK limitation.
...
Live guard: `FM_PI_BRANCH_LIVE_E2E=1 tests/fm-pi-branch-live-e2e.test.sh` exercises the real installed Pi SDK's immediate active-transcript appendEntry rendering, persistence, custom-entry model exclusion, branch-session surfaces, watcher-owned fallback after rejected branch settlement, and `message_end` replacement across streaming, stable inline and fullscreen rendering, and reopened session history.
