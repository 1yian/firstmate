...
The mate's own machinery appends to that same log: the ledger mirror and the at-source publishers in [`secondmate-parent-channel.md`](secondmate-parent-channel.md) resolve the remote route to it, so a child outcome crosses this relay whether or not the mate model wrote a line.
