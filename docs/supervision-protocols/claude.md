...
8. The turn-end guard (`bin/fm-turnend-guard.sh --claude`) remains the final backstop.
   ...
   It allows the stop when a watcher is healthy or an open identity-matched auto-arm generation claim owns recovery, while fresh failure epochs advance the bounded one-time attended fail-open progression described in [`turnend-guard.md`](../turnend-guard.md).
