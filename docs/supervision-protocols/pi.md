...
Every routine outcome remains available in the durable outcome store regardless of its optional `silent` metadata.
Normal live delivery returns it as a hidden, turn-free custom message; if interruption leaves a leading routine row unread, locked session-start recovery consumes it silently without creating a replacement message.
...
Routine outcomes require no unsolicited captain response; read the durable outcome store with the fm_branch_outcomes tool when the captain asks what happened.
