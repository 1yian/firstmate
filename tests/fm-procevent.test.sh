...
# macOS has no setsid executable; Perl's POSIX binding provides the same
# detached session before exec while keeping the observed process a sleep.
perl -MPOSIX=setsid -e 'setsid() or die "setsid: $!"; exec @ARGV or die "exec: $!"' sleep 60 &
drain_holder=$!
