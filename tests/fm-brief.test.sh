...
test_faster_paths_use_configured_authority_without_stacked_review() {
  ...
  assert_grep "append \`done [at=<epoch>]: ready in branch {your task branch}\`" "$brief" \
...
# Without a fixed prefix every ship mode lets the worker name and record a
# conventional-commit branch; the explicit override below keeps prefix support.
test_ship_branch_prefix_defaults_to_worker_choice() {
  ...
  for id_mode in "brief-branch-nm-e1:no-mistakes" "brief-branch-dp-e2:direct-PR" "brief-branch-lo-e3:local-only"; do
    ...
    brief="$home/data/$id/brief.md"
    assert_grep "fm-task-branch.sh' create '$home/state/$id.meta' <type>/<slug>" "$brief" \
      "$mode: omitting --branch-prefix must let the worker choose and record a conventional branch"
    assert_no_grep "Ship branch: fm/$id" "$brief" \
      "$mode: omitting --branch-prefix must not fix a legacy branch"
  done
  pass "fm-brief.sh: --branch-prefix omitted lets workers name and record branches"
}
