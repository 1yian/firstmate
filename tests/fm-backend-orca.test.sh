...
test_send_text_submit_verifies_empty_composer_after_enter() {
  ...
  [ "$out" = empty ] || fail "a landed plain Orca capture must confirm submission, got '$out'"
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-123'$'\x1f''--text'$'\x1f\x1f''--enter'$'\x1f''--json' \
    "a landed plain capture must authorize Enter"
  ...
  pass "fm_backend_orca_send_text_submit: plain capture confirms a landed payload"
}
...
test_send_text_submit_borderless_claude_confirms() {
  ...
  out=$( PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_send_text_submit term-123 "hello captain" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "a plain borderless composer must confirm a landed payload, got '$out'"
  pass "fm_backend_orca_send_text_submit: plain borderless composer confirms submission"
}
...
test_send_text_submit_retries_when_composer_stays_pending() {
  ...
  out=$( PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_send_text_submit term-123 "hello captain" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "a plain pending composer must retry through confirmation, got '$out'"
  log_text=$(cat "$LOG")
  ...
  [ "$enter_count" -eq 2 ] || fail "a plain pending composer must authorize two Enters, got $enter_count"
  pass "fm_backend_orca_send_text_submit: plain pending composer retries Enter"
}
...
test_send_text_submit_popup_autocomplete_requires_second_enter() {
  ...
  printf '{"ok":true,"result":{"terminal":{"tail":["  ╭──────────────────────────────────────╮","  │ ❯                                    │","  ╰──────────────── Composer ────────────╯","","  Enter:send"]}}}\n' > "$RESP/1.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/2.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["  ╭──────────────────────────────────────╮","  │ ❯ /compact                           │","  ╰──────────────── Composer ────────────╯","","  Enter:send"]}}}\n' > "$RESP/3.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/4.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["  ╭──────────────────────────────────────╮","  │ ❯ /compact compaction instructions   │","  ╰──────────────── Composer ────────────╯","","  Enter:send"]}}}\n' > "$RESP/5.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/6.out"
  ...
  [ "$out" = empty ] || fail "a plain popup composer must confirm after the second Enter, got '$out'"
  log_text=$(cat "$LOG")
  enter_count=$(printf '%s\n' "$log_text" | grep -c $'orca\x1fterminal\x1fsend\x1f--terminal\x1fterm-123\x1f--text\x1f\x1f--enter\x1f--json' || true)
  [ "$enter_count" -eq 2 ] || fail "a plain popup composer must authorize two Enters, got $enter_count"
  pass "fm_backend_orca_send_text_submit: plain popup composer retries Enter"
}
...
test_peek_send_and_crew_state_route_through_orca_meta() {
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-io'$'\x1f''--text'$'\x1f\x1f''--enter'$'\x1f''--json' \
    "a landed plain Orca capture must authorize Enter"
