...
# A secondmate reports its own child's review outcomes on the parent channel;
# it never owns a PR poll in the primary home. Reject an attempted poll without
# corrupting its record, then verify ordinary relaunch still works.
reset_meta
if poll_out=$(PATH="$HOME_DIR/fakebin:$PATH" FM_HOME="$HOME_DIR" FM_GUARD_GRACE=999999 \
  "$ROOT/bin/fm-pr-check.sh" ios https://github.com/example/repo/pull/1 2>&1); then
  fail "a secondmate must not arm a task PR poll in its parent's home"
fi
assert_contains "$poll_out" "is a secondmate, not a delivery lane" \
  "PR poll refusal did not name the secondmate ownership boundary"
OUT=$(run_relaunch ios claude claude-opus-5-5 medium); RC=$?
expect_code 0 "$RC" "a confirmed remote relaunch should succeed after an invalid PR poll attempt"$'\n'"$OUT"
pass "a secondmate refuses parent-owned PR polling and still relaunches"
