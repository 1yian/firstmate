...
SH
  ...
  ln -s "$ROOT/bin/fm-task-branch-lib.sh" "$fake/bin/fm-task-branch-lib.sh"
...
test_teardown_removes_tasktmp_dir() {
  ...
  local output
  output=$(FM_HOME="$fake" bash "$fake/bin/fm-teardown.sh" "$id" 2>&1) \
    || fail "teardown exited non-zero with a valid tasktmp: $output"
  [ ! -e "$task_tmp" ] \
