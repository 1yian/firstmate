...
# "arming" while the recorded pid now belongs to an unrelated live process. The
# identity the claim wrote into its own lock is the only thing that separates that
# from a real arm still running, so keep the beacon fresh here: this case must
# block on the identity leg alone, not the stuck-arming leg.
...
# The 2026-08-26 watcher flap, guard side: a hook hung mid-arm stayed alive with
# its ledger frozen at "arming" and no watcher beating, and the guard kept reading
# the live identity-matched owner as recovery under way, allowing blind stops. A
# claim stuck arming past grace with a beacon just as stale must reach the block.
