...
if section_enabled early-bind; then
  ...
  H_CONCURRENT="$HOMES/concurrent"
  new_home "$H_CONCURRENT"
...
PY
  H_TRAVERSAL="$HOMES/entrypoint-traversal"
  new_home "$H_TRAVERSAL"
...
PY
  H_MANIFEST_DUP="$HOMES/manifest-duplicate"
  new_home "$H_MANIFEST_DUP"
...
open(p, 'w').write(json.dumps(data))
PY
  H_MANIFEST_UNKNOWN="$HOMES/manifest-unknown"
  new_home "$H_MANIFEST_UNKNOWN"
...
PY
  ...
  if section_enabled matrix-runtime; then
    ...
    H_CORE_REPLAY="$HOMES/core-replay"
    new_home "$H_CORE_REPLAY"
...
if section_enabled lifecycle-lock; then
  ...
  H_RETIRE_RACE="$HOMES/retire-race"
  new_home "$H_RETIRE_RACE"
  touch "$race_release"
  ...
  H_PROCESS_RETIRE_RACE="$HOMES/process-retire-race"
  new_home "$H_PROCESS_RETIRE_RACE"
  ...
  H_LOCK_OWNER="$HOMES/lock-owner"
  new_home "$H_LOCK_OWNER"
  ...
  H_SIGNAL_LOCK="$HOMES/signal-lock"
  new_home "$H_SIGNAL_LOCK"
...
if section_enabled lifecycle-runner; then
  P_FLOW="$PACKAGES/flow"
  make_package "$P_FLOW" org.example.flow ext-flow
  H_ACTIVE_RUNNER="$HOMES/active-runner"
  new_home "$H_ACTIVE_RUNNER"
...
if section_enabled lifecycle-state; then
  P_FLOW="$PACKAGES/flow"
  make_package "$P_FLOW" org.example.flow ext-flow
  H_OWNER_SAFE="$HOMES/owner-safe"
  new_home "$H_OWNER_SAFE"
  ...
  H_STATE_OVERRIDE="$HOMES/state-override"
  new_home "$H_STATE_OVERRIDE"
  ...
  H_LEGACY_LINK="$HOMES/legacy-link"
  new_home "$H_LEGACY_LINK"
  ...
  H_LEGACY="$HOMES/legacy"
  mkdir -p "$H_LEGACY/state"
...
if section_enabled lifecycle-invocation-cleanup; then
  ...
  H_INVOCATION_CLEANUP="$HOMES/invocation-cleanup"
  new_home "$H_INVOCATION_CLEANUP"
...
if section_enabled example; then
  ...
  H_HANDSHAKE_ORPHAN="$HOMES/handshake-orphan"
  new_home "$H_HANDSHAKE_ORPHAN"
